...
class TvVH(parent: ViewGroup) : BaseVH<UITv>(parent.inflate(R.layout.item_tv)) {
    ...
    private val tvTvRating: TextView = itemView.findViewById(R.id.tvTvRating)

    override fun bind(item: UITv) {
        tvTvTitle.text = item.name
        ivTvPoster.downloadPoster(item.posterPath)
        tvTvRating.isVisible = item.voteAverage != null
        tvTvRating.text = DecimalFormat("#.0").format(item.voteAverage)
    }
}
